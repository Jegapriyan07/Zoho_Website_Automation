...
$(document).ready(function () {
    function setupAnimationForSection(sectionId) {
        let intervalId;

        function startAnimation($section) {
            clearInterval(intervalId);
            intervalId = setInterval(() => cycleAccWrap($section), 8000);
        }

        function cycleAccWrap($section) {
            const $current = $section.find('.acc-wrap.current');
            const $next = $current.next('.acc-wrap');
            ...
            if ($next.length) {
                ...
                $section.find('.acc-wrap:first').addClass('current');
            }
        }
            ...
            const observer = new IntersectionObserver((entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        ...
                        $target.find('.acc-wrap').removeClass('current');
                        $target.find('.acc-wrap:first').addClass('current');
        ...
        // Restart animation on manual click
        $(`${sectionId} .acc-wrap`).on('click', function () {
            clearInterval(intervalId);
            const $this = $(this);
            const $section = $this.closest(sectionId);

            $section.find('.acc-wrap').removeClass('current');
            $this.addClass('current');

            setTimeout(() => startAnimation($section), 1000);
        });
    }

    setupAnimationForSection('#sw-section');
});
...
$(document).ready(function () {
    ...
    var elmOffset = $('#right-content').offset().top;
    fixedNav(elmOffset);
...
function pageMenu() {
    $('.cont-sec').each(function () {
        ...
        if (($scrollTop + $headerHeight) >= (jQuery(this).offset().top - 60)) {
            var $id = $(this).attr('id');

            if ($id) {
                if ($('#tabs li a[href$="#' + $id + '"]').length > 0) {
                    var $position = $('#tabs li a[href$="#' + $id + '"]').position().top;
...
function fixedNav(elm) {
    if ($('#tabs').length > 0) {
        ...
        if (wW > 992) {
            ...
            var elmFixed = $('#tabs');
            if (scroll >= elm) {
                elmFixed.addClass('fixed');
            } else {
                elmFixed.removeClass('fixed');
            }

            var scrollCompletes = ($('.tabsection').outerHeight() + $('.tabsection').offset().top) - 400;
            if (scroll >= scrollCompletes) {
